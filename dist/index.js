...
let matrix = [];
function drawDiamond(matrixSize, diamondSize) {
    ...
    for (let i = 0; i < matrixSize; i++)
        matrix[i] = Array(matrixSize).fill(' ');
    try {
        // Initializing default starting positions
        let rowCenter = Math.floor((matrixSize - 1) / 2);
        let colCenter = Math.floor((matrixSize - 1) / 2);
